...
func main() {
	...
	extensionRepo := repository.NewDeadlineExtensionRepository(db)
	...
	wishSvc := service.NewWishService(wishRepo, claimRepo, extensionRepo, blessRepo, userRepo, badgeSvc, auditSvc, logger)
	claimSvc := service.NewWishClaimService(txManager, wishRepo, claimRepo, userRepo, badgeSvc, auditSvc, logger)
	extensionSvc := service.NewDeadlineExtensionService(txManager, extensionRepo, wishRepo, claimRepo, userRepo, auditSvc, logger)
	...
	handlers := &router.Handlers{
		...
		Extension: handler.NewDeadlineExtensionHandler(extensionSvc),
